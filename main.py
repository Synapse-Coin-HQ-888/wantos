...
class TimedToken:
    ...
    loudness: float   # normalized to [0.0, 1.0]
...
class AppConfig:
    """
    Runtime settings for the Synapse token-audio application.
    """
    song_path: Path
    prompt: str
    ...
    # Prevent __post_init__ from generating a new path when loading a saved run
...
class GenerationRun:
    """
    Container for replayable generation artifacts.
    """
    config: AppConfig
    ...
    def save(self, path: Path):
        """Persist run data into a compressed .npz archive."""
        # Split timed_tokens into parallel arrays
        ...
        # Serialize config (paths as strings)
        config_dict = {
    ...
    def load(cls, path: Path) -> 'GenerationRun':
        """Restore a run from a .npz archive."""
...
def colorize_token(token: str, loudness: float) -> str:
    """
    Map loudness to color via viridis on a black background; loudness ∈ [0,1].
    """
    cmap = cm.get_cmap('viridis')

    # Boost darker colors using gamma on sampled RGB, not on loudness.
    gamma = 0.75
...
def get_client(config: AppConfig) -> OpenAI:
    """
    Build an OpenAI-compatible client targeting LM Studio.
    """
    # 5s read timeout—if no bytes arrive within this window, raise APITimeoutError.
    timeout = Timeout(5.0)
...
def get_token_worker(client, config, prompt, temperature, result_queue):
    """
    Runs in a worker thread to isolate a potentially blocking network call.
    Requests a single streamed token and returns the first chunk (or an error)
...
def generate_all_tokens(
    ...
    """
    Token-by-token generation with virtual timestamps derived from TPS.
    """
    # Re-send entire context each step to accommodate temperature modulation.
    ...
    # Keep history bounded to reduce model stalls with some local backends.
    ...
    while True:
        if token_index >= config.max_tokens:
            logging.info(f"Reached configured token limit ({config.max_tokens}). Stopping.")
            break
        ...
        try:
            ...
            # If the worker never returns (e.g., network stall), bail out cleanly.
            ...
            if finish_reason == "stop":
                logging.info("Finish reason 'stop' received—ending generation.")
                break

        except queue.Empty:
            logging.warning("Worker thread unresponsive—aborting generation loop.")
            break
        except Exception as e:
...
def run_tui(
    ...
    """
    Launch the Rich TUI for generation and synchronized playback.
    """
    console = Console()
    log_stream = io.StringIO()
    log_console = Console(file=log_stream, force_terminal=True, width=120)

    # Fresh logging sink for each TUI run
    logging.basicConfig(
        level=logging.INFO,
        format='%(message)s',
        datefmt="[%X]",
    ...
    layout["header"].update(
        Panel(header_text, title="[bold magenta]Synapse DJ-Token[/]", border_style="magenta", box=HEAVY)
    )

    # Dedicated console for final summary output after Live ends
    ...
    try:
        ...
        with Live(layout, console=console, screen=True, refresh_per_second=60, redirect_stderr=False, transient=True) as live:
            ...
            if not tokens:
                logging.error("No tokens generated; exiting.")
                update_logs()
                return
            ...
            def redraw_playback_text(current_token_index: int):
                """Re-render text up to the given token index."""
                playback_text.plain = ""
            ...
            while not playstate.quit:
                ...
                if playstate.restart:
                    playback.stop()
                    # Trigger restart by exiting the Live loop via a controlled exception
                ...
                # --- Seeking (applies both paused/playing) ---
                ...
                if playstate.playing:
                    ...
                    if playstate.seek_by != 0:
                        ...
                        # Clamp virtual time not to exceed playback time (prevents drift)
...
def main(config: AppConfig):
    """
    High-level flow: analyze audio → visualize → generate/playback via TUI.
    """
    console = Console()
    ...
    # PREP
    # ----------------------------------------
    logging.info("Loading audio and computing loudness features...")
    ...
    if lufs_raw.shape[0] == 0:
        logging.error("Failed to load loudness frames or audio is silent.")
    ...
    logging.info("Audio features computed.")
    logging.info("Displaying loudness graphs; close the figure to continue.")
    ...
    while True:
        try:
            ...
            if args.load_run:
                ...
                break  # Exit after playback of a loaded run

            config = AppConfig(
                song_path=args.song_path,
                prompt=args.prompt,
                max_tokens=args.max_tokens,
                save_run_path=args.save_run  # May be None; __post_init__ handles default path
            )
            main(config)
        except Exception as e:
